...
if __name__ =='__main__':
    input_shape=(224,224)
    model = create_regression_model(input_shape,load_pretrained=True,freeze_body=0, weights_path='logs/000/ep069-loss0.014-val_loss0.004.h5')
    yolo = YOLO()
    test_folder = os.path.join(os.path.abspath('.'),os.path.join('data','random'))
    ...
    for img in images:
        ...
        if len(boxes)>0:
            ...
            rot_image = cv2.resize(rotate_by_vec(image,pt_sin,pt_cos),input_shape)
